...
void f_assembleSecondPass()
{
	...
	while (ptr != NULL)
	{
		...
		if (ptr->secondOperand)
		{
			...
			{
				nextPtr = ptr->next;
				//אם הצומת הבא אחרי הפקודה הוא צומת של אופרד הראשון,
				//צריך לדלג אחד ואז להכניס צומת אופרנד שני
				//האופרנד הראשון יכול להיות תוית שהרגע הכנסתי לה צומת או יכול להיות מספר מעון מידי
				...
				//בין הצומת של הפוקדה לבין הצומת הבא יש להכניס את צומת האופרנד
				else
				{
					newNode = f_OperandNewNode(ptr->address + 1, correctSymbol->address, correctSymbol->scope);
					ptr->next = newNode;
